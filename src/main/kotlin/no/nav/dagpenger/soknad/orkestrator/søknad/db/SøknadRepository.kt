...
class SøknadRepository(
    ...
    fun verifiserAtSøknadenHarEnAvTilstandene(
        søknadId: UUID,
        forventedeTilstander: List<Tilstand>,
    ) {
        transaction {
            val tilstand =
                SøknadTabell
            ...
            check(forventedeTilstander.map { it.name }.contains(tilstand)) {
                "Søknad $søknadId har en annen tilstand ($tilstand) enn forventet (${forventedeTilstander.map { it.name }})"
            }
        }
    }
